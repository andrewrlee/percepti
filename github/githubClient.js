...
export class GithubClient {
    ...
    get = async (path) => await superagent.get(`${apiRoot}${path}`).auth(this.owner, this.token).set('Accept', 'application/vnd.github.v3+json').set('User-Agent', 'superagent').then(res => res.body)
    getAbsolute = async (url) => await superagent.get(url).auth(this.owner, this.token).set('Accept', 'application/vnd.github.v3+json').set('User-Agent', 'superagent').then(res => res.body)
    ...
    #toPr = async ({ user, html_url: url, title, created_at: createdAt, updated_at: updatedAt, statuses_url: statusUrl }) => {
