...
export class CircleClient {
  ...
  getScheduledJobs = async (
    ...
    const { items, next_page_token } = await get(
      `/project/${slugPrefix}${name}/pipeline`
    );

    const pipelines = items
      .filter((p) => p.trigger.type === "schedule")
      .map(({ id, createdAt, number, status }) => ({
        id,
        createdAt,
        number,
        status,
  ...
  getScheduledJobsForProjects = async (projects, previous = 1, slugPrefix) => {
    const jobs = await Promise.all(
      projects.map((p) => this.getScheduledJobs(p, previous, slugPrefix))
    );
